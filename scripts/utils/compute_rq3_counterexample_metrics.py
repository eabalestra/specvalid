...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Compute RQ3 counterexample effectiveness metrics."
    )
    parser.add_argument(
        "--subjects-file",
    ...
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "experiments" / "rq3_counterexamples.csv"),
        help="Output CSV for per-assertion rows",
    )
    parser.add_argument(
        "--summary-output",
        default=str(REPO_ROOT / "experiments" / "rq3_counterexamples_summary.csv"),
    ...
    for subject_name, class_fq, method in subjects:
        ...
        for model_dir in models_dir.iterdir():
            ...
            compiled_specs_by_model[model_id] = parse_compiled_specs(
                compiled_tests_path, class_src, method
            )
        ...
        for row in log_rows:
            ...
            filtered = assertion in filtered_set
            ...
            if has_test:
                ...
                if compiled:
                    ...
                    if filtered:
                        summary_counts[summary_key]["failed_filtered"] += 1
    ...
    with summary_path.open("w", newline="") as handle:
        ...
        for (subject_id, model_id, prompt_id), counts in sorted(summary_counts.items()):
            ...
            failed_filtered = counts["failed_filtered"]
            success_rate_failed = failed_filtered / failed_total if failed_total else ""
            success_rate_with_test = (
                failed_filtered / failed_with_test if failed_with_test else ""
            )
            compile_rate = (

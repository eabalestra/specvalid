...
def classify(verdict: str, gt_status: str) -> str:
    if gt_status == "unknown":
        return "UNKNOWN_GT"
    if verdict == "FAILED" and gt_status == "invalid":
        return "TP"
    if verdict == "FAILED" and gt_status == "valid":
        return "FP"
    if verdict == "OK" and gt_status == "valid":
        return "TN"
    if verdict == "OK" and gt_status == "invalid":
        return "FN"
    return "UNKNOWN"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Compute RQ2 metrics using ground truth validity from candidate-invariant-checkers."
    )
    parser.add_argument(
        "--subjects-file",
    ...
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "experiments" / "rq2_verdicts_groundtruth.csv"),
        help="Output CSV for per-assertion rows",
    )
    parser.add_argument(
        "--summary-output",
        default=str(REPO_ROOT / "experiments" / "rq2_verdicts_groundtruth_summary.csv"),
    ...
    summary_counts = defaultdict(
        lambda: {"TP": 0, "FP": 0, "TN": 0, "FN": 0, "UNKNOWN_GT": 0}
    )
    ...
    for entry in subjects:
        ...
        gt_sets = ground_truth.get(subject_id, {"valid": set(), "invalid": set()})
        ...
        for row in log_rows:
            ...
            if assertion in gt_sets["invalid"]:
                gt_status = "invalid"
            elif assertion in gt_sets["valid"]:
                gt_status = "valid"
            else:
                gt_status = "unknown"

            label = classify(verdict, gt_status)
            ...
            if label in summary_counts[summary_key]:
                summary_counts[summary_key][label] += 1
            else:
                summary_counts[summary_key]["UNKNOWN_GT"] += 1

...
assertions_file = sys.argv[
    9
]  # Csv file containing all the assertions inferred (Step 1)
invs_by_mutants_file = sys.argv[
    10
]  # File containing specs with the mutants they kill (Step 2.1)
...
def is_non_inv_line(curr_line):
    return (
        search(":::OBJECT", curr_line)
        or search("==============", curr_line)
        or search(":::EXIT", curr_line)
        or search(":::ENTER", curr_line)
    )
    ...
    with open(assert_file) as f:
        ...
        for line in f:
            ...
            if is_non_inv_line(curr_line):
                if search(":::OBJECT", curr_line) or (
                    search(postcondition_delimiter, curr_line)
                    and search(":::EXIT", curr_line)
                ):
                    count_enabled = True
                else:
                    count_enabled = False
